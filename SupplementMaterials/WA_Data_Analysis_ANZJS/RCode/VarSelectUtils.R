...
################################################################################################################
# Get Interaction terms for the discretized model fit                                                          #
###############################################################################################################
get_interaction_terms <- function(covars_wa_massive_df){
  # Interaction between KERB_L and others
  covars_wa_massive_df$SPLI_SPEEDxKERB_L <- (covars_wa_massive_df$SPLI_SPEED)*(covars_wa_massive_df$KERB_L)
  covars_wa_massive_df$HOAL_CURVExKERB_L <- (covars_wa_massive_df$HOAL_CURVE)*(covars_wa_massive_df$KERB_L)
  covars_wa_massive_df$TOTAL_PAVExKERB_L <- (covars_wa_massive_df$TOTAL_PAVE)*(covars_wa_massive_df$KERB_L)
...
get_interaction_terms_for_marked_pattern <- function(X){
  X0 <- X[-1] # without marks column
  X0 <- get_interaction_terms(X0)
  names0 <- names(X0)
  names1 <- paste0("MARKSx", names0)
  marks <- as.integer(factor(X$marks, levels = c("Low", "High"), labels = c(0, 1))) -1
  #########################################################################################
...
number_of_nonzero_coef <- function(coef){
  return(sum(coef[,1] != 0.0)-1)
}


get_fraction_deviance_explained <- function(X, gamma){
  fit <- X$glmnet.fit
  lam <- fit$lambda
  devfrac <- fit$dev.ratio
  n <- length(gamma)
  retVec <- vector(mode="numeric", length=n)
  for(j in 1:n){
    gam <- gamma[j]
    id <- which(gam >= lam)[1]
    dfrac <- 0
    if(id > 1){
     dfrac <- mean(c(devfrac[id-1], devfrac[id])) * 100
    }else{
     dfrac <- devfrac[id] * 100
    }
    retVec[j] <- dfrac
  }
  return(retVec)
...
get_max_deviance <- function(X){
  return(max(X$glmnet.fit$dev.ratio)*100)
}

get_model_fit_info <- function(X, method = "lasso", approximation = "B-T"){
  Gamma <- c("min", "1se", "avg", "gmean")
  nGam <- length(Gamma)
  Method <- rep(method, nGam)
  Approximation <- rep(approximation, nGam)
  
  lambdaMin <- X$lambda.min
  lambda1Se <- X$lambda.1se
  lambdaAvg <- mean(c(lambdaMin, lambda1Se))
  lambdaGmean <- sqrt(lambdaMin*lambda1Se)
  
  logGamma <- c(log(lambdaMin), log(lambda1Se), log(lambdaAvg), log(lambdaGmean))
  fracDevExplained <- get_fraction_deviance_explained(X, c(lambdaMin, lambda1Se, lambdaAvg,  lambdaGmean))
  numberOfVars <- c(number_of_nonzero_coef(coef(X, s=lambdaMin)),
                    number_of_nonzero_coef(coef(X, s=lambda1Se)),
                    number_of_nonzero_coef(coef(X, s=lambdaAvg)),
                    number_of_nonzero_coef(coef(X, s=lambdaGmean)))
  
  fracExplainedDf <- data.frame(Method=Method, 
                                Approximation = Approximation,
                                Gamma = Gamma,
                                logGamma = logGamma,
                                Deviance = fracDevExplained,
                                MaxDeviance = rep(get_max_deviance(X), nGam),
                                NumberOfVariables = numberOfVars)
  return(fracExplainedDf)
}
